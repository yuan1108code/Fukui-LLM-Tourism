...
def load_config() -> APIConfig:
    ...
    # 先嘗試從環境變數讀取 GOOGLE_MAPS_API_KEY，如果沒有則嘗試 GOOGLE_API_KEY
    api_key = os.getenv("GOOGLE_MAPS_API_KEY") or os.getenv("GOOGLE_API_KEY", "")
    
    if not api_key:
        logging.error("\n❌ 未設定 Google Maps API 金鑰")
        logging.error("請檢查 .env 檔案中是否包含以下設定之一：")
        logging.error("1. GOOGLE_MAPS_API_KEY=你的API金鑰")
        logging.error("2. GOOGLE_API_KEY=你的API金鑰")
        logging.error("或者設定環境變數: export GOOGLE_MAPS_API_KEY=你的API金鑰")
